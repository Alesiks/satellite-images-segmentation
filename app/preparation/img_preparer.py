...
from app.config.main_config import BUILDINGS_DATA_PATH, IMAGE_FORMAT, BUILDINGS_MASK_DATA_PATH, \
    ...
    VALIDATION_OUTPUT_DATA_PATH, TRAIN_OUTPUT_DATA_PATH, ROTATION_ANGLE_STEP
from app.domain.img_models import SourceAndMaskImagesDto, ImageDto, DatasetPreparationSettings
...
class InriaDatasetPreparer():
    def __init__(self,
        ...
        self.imageUploader = LocalImageUploader()
        ...
        for f in filenames:
            if f.endswith(IMAGE_FORMAT):
                ...
                if self.dataset_settings.flip is True:
                    flipped_images = self.image_augumentator.flip_list(cropped_images_list)
                    cropped_images_list.extend(flipped_images)

                if self.dataset_settings.rotate > 0:
                    rotated_images = self.image_augumentator.rotate_list(cropped_images_list, self.dataset_settings.rotate)
                    cropped_images_list.extend(rotated_images)

            for source_and_mask in cropped_images_list:
                self.imageUploader.save(source_and_mask.source, self.train_dataset_path)
                self.imageUploader.save(source_and_mask.mask, self.train_dataset_path + "\\mask\\")
