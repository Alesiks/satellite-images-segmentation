...
from app.images.img_cropper import ImagesCropper
...
class ImagesPreparer(object):
    ...
    def create_data(self, data_set_size, is_validation=False):
        for f in self.filenames:
            if f.endswith(IMAGE_FORMAT):
                image_id = f[:-4]
                original_image = tiff.imread((BUILDINGS_DATA_PATH + '{}' + IMAGE_FORMAT).format(image_id)).transpose([1, 2, 0])
